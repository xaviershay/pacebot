...
(function(Opal) {
  Opal.dynamic_require_severity = "error";
  var self = Opal.top, $scope = Opal, nil = Opal.nil, $breaker = Opal.breaker, $slice = Opal.slice, $klass = Opal.klass, $hash = Opal.hash, $module = Opal.module;

  Opal.add_stubs(['$match', '$captures', '$to_f', '$map', '$to_proc', '$split', '$[]', '$new', '$to_seconds', '$==', '$+', '$to_s', '$size', '$*', '$<', '$%', '$round', '$>=', '$/', '$gsub', '$format_distance', '$distance', '$format_duration', '$seconds', '$identifier', '$raise', '$miles']);
  return (function($base, $super) {
    function $Pacebot(){};
    var self = $Pacebot = $klass($base, $super, 'Pacebot', $Pacebot);

    var def = self.$$proto, $scope = self.$$scope;

    Opal.cdecl($scope, 'MILE_MATCHER', "mi(?:les?)?");

    Opal.cdecl($scope, 'KM_MATCHER', "(?:kays|km?s?)");

    Opal.cdecl($scope, 'DURATION_MATCHER', "((\\d?:)?\\d?\\d:\\d{2})");

    def.$parse = function(msg) {
      var $a, $b, $c, self = this, distance_matcher = nil, pace_matcher = nil, match = nil, duration = nil, dist_type = nil, calc = nil, time = nil, dist = nil, mappings = nil, klass = nil, _ = nil, distance = nil;

      distance_matcher = (new RegExp("(\\d+(?:\\.\\d+)?)\\s*(" + $scope.get('MILE_MATCHER') + "|" + $scope.get('KM_MATCHER') + ")\\s+(@|in)\\s+" + $scope.get('DURATION_MATCHER')));
      pace_matcher = (new RegExp("\\b" + $scope.get('DURATION_MATCHER') + "\\s?(" + $scope.get('MILE_MATCHER') + "|" + $scope.get('KM_MATCHER') + ")"));
      if ((($a = match = distance_matcher.$match(msg)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = match.$captures())['$to_a'] && !$a['$to_a'].$$stub ? ($a = $a['$to_a']()) : ($a).$$is_array ? $a : ($a = [$a]), duration = ($a[0] == null ? nil : $a[0]), dist_type = ($a[1] == null ? nil : $a[1]), calc = ($a[2] == null ? nil : $a[2]), time = ($a[3] == null ? nil : $a[3]);
        dist = duration.$to_f();
        time = ($a = ($b = time.$split(":")).$map, $a.$$p = "to_i".$to_proc(), $a).call($b);
        mappings = $hash(["@", "m"], (($scope.get('Response')).$$scope.get('MilesAtPace')), ["in", "m"], (($scope.get('Response')).$$scope.get('MilesInDuration')), ["@", "k"], (($scope.get('Response')).$$scope.get('KmAtPace')), ["in", "k"], (($scope.get('Response')).$$scope.get('KmInDuration')));
        klass = mappings['$[]']([calc, dist_type['$[]'](0)]);
        return klass.$new(dist, $scope.get('Pacebot').$to_seconds(time));
      } else if ((($a = match = pace_matcher.$match(msg)) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = match.$captures())['$to_a'] && !$a['$to_a'].$$stub ? ($a = $a['$to_a']()) : ($a).$$is_array ? $a : ($a = [$a]), duration = ($a[0] == null ? nil : $a[0]), _ = ($a[1] == null ? nil : $a[1]), dist_type = ($a[2] == null ? nil : $a[2]);
        time = ($a = ($c = duration.$split(":")).$map, $a.$$p = "to_i".$to_proc(), $a).call($c);
        klass = (function() {if (dist_type['$[]'](0)['$==']("m")) {
          return (($scope.get('Response')).$$scope.get('MilePace'))
          } else {
          return (($scope.get('Response')).$$scope.get('KmPace'))
        }; return nil; })();
        return klass.$new($scope.get('Pacebot').$to_seconds(time));
      } else if ((($a = match = (new RegExp("[^:]\\b(\\d+(?:\\.\\d+)?)\\s*(" + $scope.get('MILE_MATCHER') + "|" + $scope.get('KM_MATCHER') + ")")).$match(" "['$+'](msg))) !== nil && (!$a.$$is_boolean || $a == true))) {
        ($a = match.$captures())['$to_a'] && !$a['$to_a'].$$stub ? ($a = $a['$to_a']()) : ($a).$$is_array ? $a : ($a = [$a]), distance = ($a[0] == null ? nil : $a[0]), dist_type = ($a[1] == null ? nil : $a[1]);
        klass = (function() {if (dist_type['$[]'](0)['$==']("m")) {
          return (($scope.get('Response')).$$scope.get('MileDistance'))
          } else {
          return (($scope.get('Response')).$$scope.get('KmDistance'))
        }; return nil; })();
        return klass.$new(distance.$to_f());
        } else {
        return nil
      };
    };

    def.$format = function(response) {
      var self = this;

      return response.$to_s();
    };

    Opal.defs(self, '$to_seconds', function(time) {
      var self = this;
    ...
    Opal.defs(self, '$format_duration', function(seconds) {
      var self = this;

      if (seconds['$<'](120)) {
        return "%is"['$%'](seconds.$round())
      } else if (seconds['$>=']((60)['$*'](60))) {
        return "%i:%02i:%02i"['$%']([seconds['$/'](60)['$/'](60), seconds['$/'](60)['$%'](60), seconds['$%'](60)])
        } else {
        return "%i:%02i"['$%']([seconds['$/'](60), seconds['$%'](60)])
      };
    });

    Opal.defs(self, '$format_distance', function(distance) {
      var self = this;

      return ("%.1f"['$%'](distance)).$gsub(/\.0$/, "");
    });

    return (function($base) {
      var self = $module($base, 'Response');

      var def = self.$$proto, $scope = self.$$scope, $a, $b, TMP_1, $c, TMP_2, $d, TMP_3, $e, TMP_4, $f, TMP_5, $g, TMP_6;

      Opal.cdecl($scope, 'MILE_RATIO', 1.609);

      Opal.cdecl($scope, 'LAP_RATIO', 0.4);

      Opal.cdecl($scope, 'MileDistance', ($a = ($b = $scope.get('Struct')).$new, $a.$$p = (TMP_1 = function(){var self = TMP_1.$$s || this;

      return (Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s mi = %s km"['$%']([$scope.get('Pacebot').$format_distance(self.$distance()), $scope.get('Pacebot').$format_distance(self.$distance()['$*']($scope.get('MILE_RATIO')))]);
        }), nil) && 'to_s'}, TMP_1.$$s = self, TMP_1), $a).call($b, "distance"));

      Opal.cdecl($scope, 'KmDistance', ($a = ($c = $scope.get('Struct')).$new, $a.$$p = (TMP_2 = function(){var self = TMP_2.$$s || this;

      return (Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s km = %s mi"['$%']([$scope.get('Pacebot').$format_distance(self.$distance()), $scope.get('Pacebot').$format_distance(self.$distance()['$/']($scope.get('MILE_RATIO')))]);
        }), nil) && 'to_s'}, TMP_2.$$s = self, TMP_2), $a).call($c, "distance"));

      Opal.cdecl($scope, 'MilePace', ($a = ($d = $scope.get('Struct')).$new, $a.$$p = (TMP_3 = function(){var self = TMP_3.$$s || this;

      return (Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s mile = %s km = %s lap"['$%']([$scope.get('Pacebot').$format_duration(self.$seconds()), $scope.get('Pacebot').$format_duration(self.$seconds()['$/']($scope.get('MILE_RATIO'))), $scope.get('Pacebot').$format_duration(self.$seconds()['$/']($scope.get('MILE_RATIO'))['$*']($scope.get('LAP_RATIO')))]);
        }), nil) && 'to_s'}, TMP_3.$$s = self, TMP_3), $a).call($d, "seconds"));

      Opal.cdecl($scope, 'KmPace', ($a = ($e = $scope.get('Struct')).$new, $a.$$p = (TMP_4 = function(){var self = TMP_4.$$s || this;

      return (Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s km = %s mile = %s lap"['$%']([$scope.get('Pacebot').$format_duration(self.$seconds()), $scope.get('Pacebot').$format_duration(self.$seconds()['$*']($scope.get('MILE_RATIO'))), $scope.get('Pacebot').$format_duration(self.$seconds()['$*']($scope.get('LAP_RATIO')))]);
        }), nil) && 'to_s'}, TMP_4.$$s = self, TMP_4), $a).call($e, "seconds"));

      Opal.cdecl($scope, 'AtPace', ($a = ($f = $scope.get('Struct')).$new, $a.$$p = (TMP_5 = function(){var self = TMP_5.$$s || this;

      Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s%s @ %s pace = %s"['$%']([self.$distance().$to_s(), self.$identifier(), $scope.get('Pacebot').$format_duration(self.$seconds()), $scope.get('Pacebot').$format_duration(self.$seconds()['$*'](self.$distance()))]);
        });
        return (Opal.defn(self, '$identifier', function() {
          var self = this;

          return self.$raise("Must implement in subclass");
        }), nil) && 'identifier';}, TMP_5.$$s = self, TMP_5), $a).call($f, "distance", "seconds"));

      (function($base, $super) {
        function $MilesAtPace(){};
        var self = $MilesAtPace = $klass($base, $super, 'MilesAtPace', $MilesAtPace);

        var def = self.$$proto, $scope = self.$$scope;

        return (def.$identifier = function() {
          var self = this;

          return "mi";
        }, nil) && 'identifier'
      })(self, $scope.get('AtPace'));

      (function($base, $super) {
        function $KmAtPace(){};
        var self = $KmAtPace = $klass($base, $super, 'KmAtPace', $KmAtPace);
      ...
      Opal.cdecl($scope, 'InDuration', ($a = ($g = $scope.get('Struct')).$new, $a.$$p = (TMP_6 = function(){var self = TMP_6.$$s || this;

      return (Opal.defn(self, '$to_s', function() {
          var self = this;

          return "%s%s @ %s pace = %s"['$%']([self.$miles().$to_s(), self.$identifier(), $scope.get('Pacebot').$format_duration((self.$seconds()['$/'](self.$miles().$to_f())).$round()), $scope.get('Pacebot').$format_duration(self.$seconds())]);
        }), nil) && 'to_s'}, TMP_6.$$s = self, TMP_6), $a).call($g, "miles", "seconds"));

      (function($base, $super) {
        function $MilesInDuration(){};
        var self = $MilesInDuration = $klass($base, $super, 'MilesInDuration', $MilesInDuration);
      ...
      })(self, $scope.get('InDuration'));

      (function($base, $super) {
        function $KmInDuration(){};
        var self = $KmInDuration = $klass($base, $super, 'KmInDuration', $KmInDuration);
